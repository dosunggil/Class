...
public class ControllerV2 {
	...
	public static void main(String[] args) {
		// ServiceV2 에는 기본 생성자가 없다.
		// 그 이유는 ServiceV2 에서는 내부에서 사용하는
		// intList 를 초기화 하는 코드가 없고
		// ServiceV2 를 호출하는 외부에서(Controller) 초기화된 intList 를 매개변수로 주입해서 사용해야하기 때문.
		
		// 만약 기본 생서앚를 호출하여 객체를 만들게되면 
		// intList 에서 Nullpoint Exception 이 발생한다.
		// 그러한 이유로 기본 생성자를 아예 호출하지 못하도록 만든다.
		
		// 기본 생성자에서 intList 를 초기화하는 코드를 추가할 수 있지만,
