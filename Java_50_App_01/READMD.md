# interface arguments
* 생성자, method 를 설계할 때 interface 가 있는 type 으로 argumenet 를 작성하면 클래스와 클래스같의 결합도를 낮추어 프로젝트가 커졌을 때 많은 장점을 얻을 수 있다.

* 다음과 같이 class를 매개변수로 갖는 method 를 설계할 때
```
public void method(ArrayList<Integer> list) {}
```
* 그리고 다음과 같이 interface 를 매개변수로 갖는 method 를 설계할 때의 차이
```
public void method(List<Integer> list) {}
```
* 첫 번째 코드에서는 method를 다음과 같은 코드로밖에 사용하지 못한다.
```
...
* 하지만 두 번째 코드에서는 다음처럼 유연한 방법으로 method 를 사용할 수 있다.
```
...
* 클래스에서 사용하는 ```데이터 객체```, 또는 ```서비스 객체``` 등을 클래스 코드에서 직접 초기화 하지 않고, 생성자 또는 setter() 등을 통하여 주입받는 방식
...
* 전달받은 method 에서 ```요소의 값``` 을 변경 (추가, 수정, 삭제) 하면 전달해준 곳의 값도 같이 변경된다.
